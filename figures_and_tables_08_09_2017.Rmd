...
```{r figure3a, echo=FALSE, cache=TRUE}
##################################################
...
```{r figure3b, echo=FALSE, cache=TRUE}
##################################################
# Read saved results of simulations achieving API
##################################################
...
**FIGURE 4: API as a function of follow up.** Percentages of simulations (n=100) achieving joint API for all four model parameters in a stop-screen trial with 50,000 women screened annually at ages 50--54 years, by years of follow-up after the last screen. Mean sojourn time of (A) 6 months and (B) 4 years. Lines connect evaluations under $\psi$ set equal to (dark) 20%, (medium) 40%, and (light) 60%, assuming a constant rate of onset of preclinical cancer of $w=0.0025$ per year, and sensitivity of screening to detect preclinical cancer of $\beta=80\%$.
...
```{r figure4, cache=TRUE, echo=FALSE}
##################################################
...
**FIGURE 5: Identifiability as a function of follow up.** Profile likelihoods of $\psi$ and $\lambda$ for representative realizations of the $\psi=60\%$ scenario in Figure 3A. With 0 years follow up (black lines), $\lambda$ is practically non-identifiable. With 2 years follow up (dark grey lines), $\lambda$ is practially identifiable, but does not satisfy API. With 4 and 6 years of follow up (grey and light grey lines), $\lambda$ is clearly API. The remaining parameters $\beta$ and $w$ are API under all considered follow up scenarios (results not shown). Simulation parameters as follows: $n=50,000$ trial participants, $\lambda=2$, $w=0.0025$ and $\beta=80\%$.
...
mis.dat$w<-1000*mis.dat$w
##################################################
# Illustrate estimates under mis-specified model
##################################################
mpanel <- function(x.vec, dset, parname){
...
grid.arrange(mpanel(1000*x.vec, mis.dat, 'w'),
             mpanel(x.vec, mis.dat, 'lambda'),
             mpanel(x.vec, mis.dat, 'beta'),
             layout_matrix=matrix(c(1, 2, 3), ncol=3, byrow=TRUE))
```

**FIGURE 6: Model misspecification.** Violin plots of maximum likelihood estimates for parameters in a progressive model (i.e., one that assumes $\psi=0$) fit to data generated using a mixture model with selected values of $\psi\ge0$ assuming 50,000 women were screened annually at ages 50--54 with follow-up to age 60 years and the screening test has 80% sensitivity. Dashed horizontal lines indicate true parameter values. Results based on n=200 simulations per $\psi$-value. $w$ scaled by a factor of $10^3$.

\newpage

```{r figure-cnbss-profile-likelihoods, results='asis'}
##################################################
...
# Initial values and limits for optimization algorithm
##################################################
x0 <- c(psi=0.2, lambda=.05, w=1/1000, beta=.6)
xl <- c(psi=0, lambda=0, w=0, beta=0)
...
**Figure 7: Profile likelihood for CNBSS-2: mixture model.**  Relative negative log-likelihoods (NLL) based on fitting the mixture model to CNBSS-2 data (see Table S1). Vertical dashed lines correspond to maximum likelihood estimates. Intersection of relaitve NLL with horizontal dotted lines indicate the profile-based  95% confidence intervals. 
...
cnbprint <- cbind(cnbprint, cnbss50)
colnames(cnbprint) <- c('Round', 'Screen', 'Interval', 'n')
print(xtable(cnbprint, digits=0),
...
**Table S1: CNBSS-2 data.** Grouped data from the Canadian Breast Cancer Screening Study-2 [Miller et al., CMAJ, 1992]. Round: screening round; Screen: number of screen-detected cases; Interval: number of interval cases; n: number of women who attended all screening rounds up to and including the current round.
...
# to sample size
##################################################
sizeplot <- function(dset, par1, par2, response){
  melted <- melt(dset, id.vars=c(par1, par2, 'followup'))
  melted <- transform(melted,
...
```{r sample-size-bias, echo=FALSE}
##################################################
# Illustrate sensitivity of bias to follow-up
##################################################
sizeplot(dset, par1='psi', par2='lambda', response='bias')
```

**FIGURE S1: Sensitivity of bias to follow up.** Absolute bias for all four model parameters assuming 50,000 women were screened annually at ages 50--54 years with follow-up to age 55 and 60 years, respectively. Screening test sensitivity was   set to $\beta=80\%$.

\newpage

```{r sample-size-se, echo=FALSE}
##################################################
# Illustrate sensitivity of SEs to follow-up
##################################################
sizeplot(dset, par1='psi', par2='lambda', response='SE')
```

**FIGURE S2: Sensitivity of the standard error to follow up.** Standard errors for all four model parameters assuming 50,000 women were screened annually at ages 50--54 years with follow-up to age 55 and age 60 yearsk respectively. Screening test sensitivity was   set to $\beta=80\%$.
...
<!-- ################################################## -->
<!-- #grid.draw(progressive) # Roman - I can't find this function -->
<!-- plot(progressive) -->
<!-- ``` -->

<!-- **Figure S3: Fitting progressive model to combined data from CNBSS-I and CNBSS-II.** Profile likelihoods for the three model parameters are shown assuming 50,000 women were screened annually at ages 50--54 with follow-up to age 55 years (Table S1). -->
...
<!-- **Figure S4: Fitting mixture model to combined data from CNBSS-I and CNBSS-II.** Profile likelihoods for the four mixture model parameters are shown assuming 50,000 women were screened annually at ages 50--54 with follow-up to age 55 years (Table S1). -->
...
**FIGURE S3: Model misspecification.** Violin plots of maximum likelihood estimates for parameters in a progressive model (i.e., one that assumes $\psi=0$) fit to data generated using a mixture model with selected values of $\psi\ge0$ assuming 50,000 women were screened annually at ages 50--54 with follow-up to age 60 years and the screening test has 40% sensitivity. Results based on n=200 simulations per $\psi$-value. Axes for $w$ scaled by a factor of $10^3$.
...
# Here, we fix psi at different values and find the MLE and GoF
...
# MLE
##################################################
for(k in 1:dim(table_cnbss.1)[1]){
  #########
  # MLE
  #########
...
# Output
##################################################
colnames(table_cnbss.1) <- c('$\\psi$',  '$\\lambda$', '$\\lambda$ lr', '$\\lambda$ up', '$w$', '$w$ lr', '$w$ up', '$\\beta$', '$\\beta$ lr', '$\\beta$ up', '$\\chi^2$', 'p-val')

table_cnbss.1$`$w$`<-1000*table_cnbss.1$`$w$`
table_cnbss.1$`$w$ lr`<-1000*table_cnbss.1$`$w$ lr`
table_cnbss.1$`$w$ up`<-1000*table_cnbss.1$`$w$ up`

print(xtable(table_cnbss.1),
      digits=4,
      include.rownames=FALSE,
      hline.after=0,
      comment=FALSE,
...
**Table S2: CNBSS-2: parameter estimation and goodness of fit for constrained mixture model.** The mixture model with fixed fraction of indolent cancers $\psi$ is fit to the CNBSS-2 data (see Table S1). Onset of preclinical disease is assumed to be negligible before age $\Delta_0=45$ years. $lr$ and $up$ denote the lower and upper bounds of the 95% profile confidence intervals. Values for $w$ are reported per 1,000 women. $\chi^2$ is the value of the statistic, and $P-val$ the corresponding goodness of fit P-value.


\newpage 

```{r cnbss2-table2, results='asis', echo=FALSE}
# Here, vary the length of the time interval of positive onset away from 45 years
t_fix <- c(20, 15, 10, 5)
l.p <- length(t_fix)
n.y <- dim(cnbss50)[1]
...
colnames(table_cnbss.1) <- c('$\\Delta_0$', '$\\psi$', '$\\psi$ lr', '$\\psi$ up', '$\\lambda$', '$\\lambda$ lr', '$\\lambda$ up', '$w$', '$w$ lr', '$w$ up', '$\\beta$', '$\\beta$ lr', '$\\beta$ up', '$\\chi^2$', 'P-val')

table_cnbss.1$`$w$`<-table_cnbss.1$`$w$`*1000
table_cnbss.1$`$w$ lr`<-1000*table_cnbss.1$`$w$ lr`
table_cnbss.1$`$w$ up`<-1000*table_cnbss.1$`$w$ up`
...
**Table S3: CNBSS-2: Parameter estimation and goodness of fit for varying onset ages.** The mixture model is fit to the CNBSS-2 data (see Table S1) for varying ages of first possible onset of preclinical disease. $lr$ and $up$ denote the lower and upper bounds of the 95% profile confidence intervals. Values for $w$ are reported per 1,000 women. $\chi^2$ is the value of the statistic, and $P-val$ the corresponding goodness of fit P-value.
