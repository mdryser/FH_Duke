...
**FIGURE 3: API as a Function of Follow Up.** Percentages of simulations (n=100) achieving joint API for all four model parameters in a stop-screen trial with 50,000 women screened annually at ages 50--54 years, by years of follow-up after the last screen. Mean sojourn time of 6 months (A) and 4 years (B), respectively. Lines connect evaluations under $\psi$ set equal to (dark) 20%, (medium) 40%, and (light) 60%, assuming a constant rate of onset of preclinical cancer of $w=0.0025$ per year, and sensitivity of screening to detect preclinical cancer of $\beta=80\%$.
...
**FIGURE 5: Model misspecification.** Violin plots of maximum likelihood estimates for parameters in a progressive model (i.e., one that assumes $\psi=0$) fit to data generated using a mixture model with selected values of $\psi\ge0$ assuming 50,000 women were screened annually at ages 50--54 with follow-up to age 60 years and the screening test has 80% sensitivity. Results based on n=200 simulations per $\psi$-value.
...
```{r CNBSS table, results='asis', echo=FALSE}
source('trials_FU.R')

cnbprint=data.frame(Round=c(1,2,3,4,5))
cnbprint<-cbind(cnbprint, cnbss50)
colnames(cnbprint)<- c('Round', 'Screen','Interval' , 'n')
print(xtable(cnbprint, digits=0), include.rownames = FALSE, hline.after=0,comment=FALSE, sanitize.colnames.function=function(x) {x})

# Like Table 1 in Shen et al. (2017); without HIP, but including 5th screening round for  CNBSS40 and CNBSS50
```
...
sizeplot <- function(dset, par1, par2, response){
  melted <- melt(dset, id.vars=c(par1, par2, 'size'))
  melted <- transform(melted,
                      measure=sub('^([^_]*)_([^_]*)$', '\\1', variable),
                      variable=sub('^([^_]*)_([^_]*)$', '\\2', variable))
  melted <- transform(melted, variable=factor(variable,
  ...
  gg <- gg+facet_grid(variable~size, labeller=label_parsed)
  gg <- gg+scale_x_continuous(name=parse(text=par1),
                              labels=percent_format(),
                              expand=c(0, 0))
  gg <- gg+scale_y_log10(name=parse(text=par2),
...
dset <- rbind(transform(result_mat_55, size='n==50000'),
              transform(result_mat_60, size='n==50000'))
```
...
**FIGURE S1: Sensitivity of bias to follow up.** Bias for all four model parameters assuming 50,000 women were screened annually at ages 50--54 years with follow-up to age 55 and 60 years, respectively. Screening test sensitivity was   set to $\beta=80\%$.
...
```{r cnbss-2 table 1, results='asis', echo=FALSE, comment=FALSE}

# Here, we fx psi at different values and find the MLE and GoF

psi_fix<-c(0,.05,.1,.2,.4)
l.p<-length(psi_fix)


table_cnbss.1<-data.frame(psi=psi_fix, lambda=numeric(l.p), lambda.CI.lr=numeric(l.p), lambda.CI.up=numeric(l.p), w=numeric(l.p), w.CI.lr=numeric(l.p), w.CI.up=numeric(l.p), beta=numeric(l.p), beta.CI.lr=numeric(l.p),beta.CI.up=numeric(l.p), statistic=numeric(l.p), p_value=numeric(l.p))
...
for(k in 1:dim(table_cnbss.1)[1]){
  ...
  X= sum((sc_ex-cnbss50[,1])^2/sc_ex) + sum((cl_ex-cnbss50[,2])^2/cl_ex ) # Xi square statistic
...
print(xtable(table_cnbss.1), digits=4, include.rownames = FALSE, hline.after=0,comment=FALSE, sanitize.colnames.function=function(x) {x})
...
```{r cnbss-2 table 2, results='asis', echo=FALSE}
...
table_cnbss.1<-data.frame(age_onset=numeric(l.p), psi=numeric(l.p), psi.CI.lr=numeric(l.p), psi.CI.up=numeric(l.p), lambda=numeric(l.p), lambda.CI.lr=numeric(l.p), lambda.CI.up=numeric(l.p), w=numeric(l.p), w.CI.lr=numeric(l.p), w.CI.up=numeric(l.p), beta=numeric(l.p), beta.CI.lr=numeric(l.p),beta.CI.up=numeric(l.p), statistic=numeric(l.p), p_value=numeric(l.p))
